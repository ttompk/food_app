...
class PicovoiceThread(Thread):
    ...
    def _context_path():
        return os.path.join(
            os.path.dirname(os.path.realpath(__file__)),
            'contexts/alarm_raspberry-pi.rhn')
    ...
    def _inference_callback(self, inference):
        ...
        if inference.is_understood:
            if inference.intent == 'setAlarm':
                ...
                self._time_label.configure(text=f'{self._hours}: {self._minutes} : {self._seconds}')
                ...
                # raise valueError here ... or ask to repeat for 2 more times?
